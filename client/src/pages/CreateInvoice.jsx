...
const CreateInvoice = () => {
  ...
  const addProduct = () => {
    ...
    if (products.length >= 20) {
      toast.error(
        "You can only add up to 20 items! Please create another invoice."
      );
      return;
    }
  ...
  const addDescription = () => {
    if (descriptions.length >= 10) {
      toast.error("You can only add up to 10 descriptions!");
      return;
    }
    ...
    <main className="w-full py-8 lg:px-8 px-5">
      ...
      <section className="flex flex-col md:flex-row items-start md:items-center gap-8 mt-8">
        <div className="bg-[#011C2B] lg:h-auto w-full md:w-1/4 py-8 px-5 rounded-xl">
          <img
            src={logo}
            className="w-[70%] mx-auto my-16 hidden md:block "
            alt="logo"
          />
          ...
          <div className="w-full mt-6 flex flex-col items-start justify-center gap-6 text-white">
            <h4 className="text-[#C9C9C9] sm:text-3xl text-xl">
          ...
          <form className="flex flex-col items-start">
            ...
            <div className="flex flex-col mt-6">
              <button
                ...
                className="bg-green-600 text-white px-6 py-3 text-2xl rounded-md mb-4"
              >
                Add Description
              </button>
              {descriptions.map((desc, index) => (
                <div key={index} className="flex flex-col mb-2">
                  <label htmlFor={`description-${index}`} className="text-2xl">
                    Description {index + 1}
                  </label>
                  <input
                    type="text"
            ...
            <div className="flex justify-start mt-6 w-full">
              <button
                type="button"
                onClick={addProduct}
                className="bg-green-600 text-white px-6 py-3 text-2xl rounded-md"
              >
                Add Product
              </button>
            </div>
